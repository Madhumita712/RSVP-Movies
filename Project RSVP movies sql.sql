...
-- Type your code below:
SELECT sum(r.total_votes) AS total_votes, m.languages
FROM movie AS m
INNER JOIN ratings AS r
ON m.id = r.movie_id
...
-- Subquery to calculate average ratings and ranks
...
-- Select the top 3 actresses based on average rating
SELECT 
    name,
    total_votes,
    movie_count,
